...
namespace ts {
    ...
    export function transformClassFields(context: TransformationContext) {
            ...
            if (shouldTransformPrivateElementsOrClassStaticBlocks) {
                ...
                for (const member of node.members) {
                    if (isPrivateIdentifierClassElementDeclaration(member)) {
                        addPrivateIdentifierToEnvironment(member, member.name);
                    }
                }
                ...
                if (shouldTransformAutoAccessors) {
                    for (const member of node.members) {
                        if (isAutoAccessorPropertyDeclaration(member)) {
                            addPrivateIdentifierToEnvironment(member, factory.getGeneratedPrivateNameForNode(member.name, /*prefix*/ undefined, "_accessor_storage"));
                        }
                    }
                }
            }
        ...
        function addPrivateIdentifierToEnvironment(node: PropertyDeclaration | MethodDeclaration | GetAccessorDeclaration | SetAccessorDeclaration, name: PrivateIdentifier) {
            const lex = getClassLexicalEnvironment();
            ...
            if (hasStaticModifier(node)) {
                Debug.assert(lex.classConstructor, "weakSetName should be set in private identifier environment");
                if (isPropertyDeclaration(node)) {
                    ...
                    const functionName = createHoistedVariableForPrivateName(name);
                    setPrivateIdentifier(privateEnv, name, {
                        kind: PrivateIdentifierKind.Method,
                ...
                Debug.assert(privateEnv.weakSetName, "weakSetName should be set in private identifier environment");

                setPrivateIdentifier(privateEnv, name, {
                    kind: PrivateIdentifierKind.Method,
                    methodName: createHoistedVariableForPrivateName(name),
                    brandCheckIdentifier: privateEnv.weakSetName,
                    isStatic: false,
                    isValid,
                });
            }
